...
class StateManager:
    ...
    def _migrate_v6(self, data: dict):
        if data.get("schema_version") != 6:
            return None
        delivery = data.get("delivery") or {}
        phase = delivery.get("phase") if isinstance(delivery, dict) else None
        is_terminal = data.get("status") in ("INITIALIZED", "COMPLETED") or not phase or phase == "TASK_COMPLETED"
        if phase in ("VALIDATING", "REVIEWING", "COMMITTING") and not is_terminal:
            raise RuntimeError(f"Legacy active review checkpoint cannot be safely resumed under schema v7. (phase={phase}) Please reset workspace or complete with previous version.")
        migrated = dict(data)
        migrated["schema_version"] = CURRENT_SCHEMA_VERSION
        return migrated
    ...
    def load(self) -> dict:
        ...
        if data is not None:
            ...
            if data.get("schema_version") == 6:
                try:
                    migrated = self._migrate_v6(data)
                    if migrated is not None:
                        self.save(migrated, event="migrated_v6_to_v7")
                        return migrated
                except RuntimeError:
                    raise
                except Exception as e:
